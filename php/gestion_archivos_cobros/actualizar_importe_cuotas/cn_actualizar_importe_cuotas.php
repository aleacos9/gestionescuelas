...
class cn_actualizar_importe_cuotas extends gestionescuelas_cn
{
    ...
    private function actualizar_importe_cuotas_adeudadas()
    {
        ...
        $sql = "UPDATE transaccion_cuenta_corriente tcc
                SET
                    importe = (SELECT CAST(valor AS numeric)
                ...
                WHERE tcc.id_alumno_cc = acc.id_alumno_cc
                    AND acc.id_cargo_cuenta_corriente = 2
    ...
    {
        $mes = new DateTime('first day of last month');
        return ucfirst(strftime('%B', $mes->getTimestamp()));
    }
}
